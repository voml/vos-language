...
const JS_STUBS = ['@game-gpt/vos', '@game-gpt/vos-skills'];

const VERSION = '0.0.0';

const TRUST = {
    repo: 'voml/vos-language',
...
function runNpm(args, opts = {}) {
    ...
    let userConfig;
    if (token) {
        userConfig = path.join(os.tmpdir(), `vos-placeholder-npmrc-${process.pid}`);
...
function writeStub(spec, dir, authToken) {
    ...
    const pkg = {
        name: spec.name,
        version: VERSION,
        description: spec.description ?? 'VOS placeholder — not for production use.',
        license: 'MIT',
        private: false,
        files: ['README.md'],
    };
    ...
    fs.writeFileSync(path.join(dir, 'README.md'), `# ${spec.name}\n\nPlaceholder package (${VERSION}). Reserved for the VOS project.\n`);
    if (authToken) {
...
function cmdPublish() {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'vos-npm-stubs-'));
