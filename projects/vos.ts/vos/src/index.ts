/**
 * Minimal VOS surface for the language homepage.
 *
 * VOS is the same schema language as Atlas Iris + Hermes:
 * `namespace` / `using` / `table` / `class` / `enums` / `flags` /
 * `union` / `service` / `const` / `obsolete`.
 */
...
const TOP_LEVEL =
    /\b(namespace|using|table|class|enums|flags|union|service|const|obsolete)\b/;
...
export function checkSource(source: string): VosCheckResult {
    ...
    if (!TOP_LEVEL.test(source)) {
        diagnostics.push({
            line: 1,
            message:
                "expected a top-level item: namespace, using, table, class, enums, flags, union, service, const, or obsolete",
        });
    }

    // Catch the Hermes positional-parameter footgun early in the playground.
    ...
    if (positional) {
        ...
        const line =
            source.slice(0, positional.index ?? 0).split(/\r?\n/).length || 1;
        diagnostics.push({
            line,
